...
describe('Service: Metadata', () => {
  ...
  it("should insert a new metadata", async () => {
    ...
    let datas = await metadataService.collection.find({}).toArray();
    expect(datas.length).to.equal(2);
    expect(datas.find((x) => x.name === secondMetadata.name)).to.deep.equals(
      secondMetadata
    );
  });
  ...
  it("should not insert a new metadata if it has an invalid name and tags", async () => {
    ...
    try {
      ...
      let courses = await metadataService.collection.find({}).toArray();
  ...
  describe("Error handling", async () => {
    ...
    it("should throw an error if we try to add metadata on a closed connection", async () => {
      ...
      expect(metadataService.addMetadata(secondMetadata)).to.eventually.be.rejectedWith(
        Error
      );
    });
  });
